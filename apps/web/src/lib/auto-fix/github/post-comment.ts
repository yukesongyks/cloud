/**
 * GitHub Issue Comment Helper (Auto Fix)
 *
 * Posts comments on GitHub issues using the GitHub REST API.
 * Used by the auto-fix system to communicate with issue reporters.
 */
...
export async function postIssueComment(params: PostIssueCommentParams): Promise<void> {
  ...
  logExceptInTest('[auto-fix:postIssueComment] Posting comment', {
    repoFullName,
    issueNumber,
  ...
  try {
    ...
    if (!response.ok) {
      ...
      errorExceptInTest('[auto-fix:postIssueComment] GitHub API error', {
    ...
    logExceptInTest('[auto-fix:postIssueComment] Comment posted successfully', {
      issueNumber,
    });
  } catch (error) {
    errorExceptInTest('[auto-fix:postIssueComment] Error posting comment:', error);
    captureException(error, {
      tags: { operation: 'auto-fix-post-issue-comment' },
