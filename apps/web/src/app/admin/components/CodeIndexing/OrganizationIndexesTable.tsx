...
export function OrganizationIndexesTable() {
  const trpc = useTRPC();
  const [page, setPage] = useState(1);
  const [pageSize, setPageSize] = useState(20);
  const [sortBy, setSortBy] = useState<OrgSortField>('size_kb');
  ...
  return (
    <div className="flex flex-col gap-y-4">
      <div>
        <h3 className="text-xl font-semibold">Organization Indexes</h3>
        <p className="text-muted-foreground text-sm">Code indexes owned by organizations</p>
      </div>
            ...
            {isLoading ? (
              ...
              <TableRow>
                <TableCell colSpan={7} className="text-muted-foreground h-24 text-center">
                  No organization code indexing data available
                </TableCell>
              </TableRow>
            ) : (
              stats.map(stat => (
                <TableRow key={stat.organization_id}>
                  <TableCell>
                    <Link
                      href={`/admin/code-indexing/${stat.organization_id}`}
                      className="text-blue-600 hover:underline dark:text-blue-400"
                      prefetch={false}
                    >
                      {stat.organization_name || 'Unknown Organization'}
                    </Link>
                  </TableCell>
                  <TableCell className="text-right">
