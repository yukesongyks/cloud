...
import { useSetWastelandPageHeader } from '@/app/(app)/wasteland/by-id/[wastelandId]/WastelandPageHeaderContext';
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export function SettingsClient() {
  ...
  const wastelandId = repo.wastelandId;
  ...
  function handleSave() {
    // Upstream is no longer editable here — it's part of the URL.
    updateConfig.mutate({
      wastelandId,
  ...
  // Active sections — filtered to match what's actually rendered so the
  // nav stays in sync with the body. Admin sections only appear for
  // admins; the danger zone only for owners. The deprecated container
  // status block from the legacy tree is intentionally not surfaced.
  ...
  // The route-segment layout (`./layout.tsx`) provides an
  // `overflow-y-auto` wrapper that scrolls at the container level.
  // The wasteland header sits above the scroll viewport, so sections
  // land at the top of this scroll root.
  const { activeId: activeSection, scrollTo: scrollToSection } = useScrollSpy(
    navSections.map(s => s.id),
    { scrollRootId: 'wasteland-repo-settings-scroll-root' }
  );

  // Contribute the Save CTA into the wasteland header. The header
  // already shows the page title ("Settings") through its subtitle
  // resolver, so we only publish the action button here.
    ...
    <div className="scroll-smooth">
      <div className="mx-auto flex max-w-4xl px-6">
          ...
          <div className="space-y-8 pt-6" style={{ paddingBottom: '75vh' }}>
            ...
            >
              <div className="space-y-4">
                ...
                {/* DoltHub upstream — read-only. The upstream is encoded in
                    the URL (`/wasteland/<owner>/<repo>`) and is no longer
                    editable from settings. To change it, create a new
                    wasteland connected to a different upstream. */}
                <FieldGroup
                  label="DoltHub Upstream"
                  hint="The DoltHub repository for this wasteland's bounty board. Set when the wasteland is connected and not editable here."
                >
                  <div className="flex items-center justify-between gap-3 rounded-md border border-white/[0.08] bg-white/[0.03] px-3 py-2">
                ...
                {/* Admin-mode toggle — self-attestation that the stored
                    token has push access on the upstream. Unlocks direct
                    writes, PR merge controls, and accept/reject in the
                    wanted board. Only shown once a credential has been
                    stored. */}
                {credential && (
        ...
        <SettingsScrollspyNav
          ...
          layoutId="wasteland-repo-settings-nav-indicator"
        />
      </div>
    </div>
  );
