...
type LeaseRow = typeof executionLeases.$inferSelect;
...
export function createLeaseQueries(db: DrizzleSqliteDODatabase, rawSql: SqlStorage) {
  return {
    /**
     * SAFETY NOTE: The check-then-set pattern used here is safe because
     * Durable Objects serialize all incoming requests within a single instance.
     */
    tryAcquire(
      executionId: string,
      leaseId: string,
    ...
    extend(
      ...
      if (existing.lease_id !== leaseId) {
        return Err({ code: 'WRONG_HOLDER', currentHolder: existing.lease_id });
      }
